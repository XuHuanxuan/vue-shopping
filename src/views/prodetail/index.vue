<template>
  <div class="prodetail">
    ...
    <div class="footer">
      ...
      <div @click="addCartBtn" class="btn-add">加入购物车</div>
      <div @click="buyNowBtn" class="btn-buy">立刻购买</div>
    </div>

    <!-- 弹层 -->
    <van-action-sheet v-model="showPannel" :title="sheetMode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
        ...
        <div class="num-box">
          <span>数量</span>
          <van-stepper v-model="addCount" min="1" integer />
        </div>
        <div class="showbtn" v-if="detail.stock_total > 0">
          <div class="btn" @click="addCart" v-if="sheetMode === 'cart'">加入购物车</div>
...
import { axiosAddCart } from '@/api/cart.js'
...
export default {
  ...
  methods: {
    ...
    async addCart () {
      ...
      const res = await axiosAddCart(this.goodsId, this.addCount, this.detail.skuList[0].goods_sku_id)
